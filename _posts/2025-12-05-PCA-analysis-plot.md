...
PCA（主成分分析，Principal Component Analysis）是一种**降维和数据压缩**方法，用来把高维、相关性强的变量，转换为少数几个**彼此正交、信息量最大的主成分**。
它通过最大化方差来保留数据中最重要的结构，从而**减少噪声、简化数据、便于可视化和建模**。
在生物信息学中，PCA常用于**群体结构分析、样本分群和异常样本检测**。

# 一、文件准备

PCA首先需要群体的数据，可以是表型或基因型矩阵。

在GWAS分析中，vcf格式的基因型文件，需要经过plink过滤后，最终转为plink格式(bed, bim, fam)，才能进行PCA群体分层。
