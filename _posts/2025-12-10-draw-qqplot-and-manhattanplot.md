...
通过PLINK, GCTA, GEMMA等软件得到的数据还需要经过可视化分析才能更好地展示我们的结果，常用的有Q-Q图和曼哈顿图。

Q-Q 图（Quantile–Quantile plot）主要判断统计结果是否可靠。它通过比较观测到的 P 值分布与理论期望分布来观察是否存在系统性偏倚。如果大多数点落在对角线附近，只有尾部明显偏离，说明可能存在真实关联信号。

曼哈顿图（Manhattan plot）用来定位显著关联位点，查看显著性SNPs位于哪条染色体上的什么位置。
